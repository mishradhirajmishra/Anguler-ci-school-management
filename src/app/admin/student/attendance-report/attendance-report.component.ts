...
export class AttendanceReportComponent implements OnInit {
 ...
 this.as.chkLogedIn({ 'token': this.token }).subscribe(
  ...
  getAttendance(){
    this.as.studentAttendanceReport({'class_id':this.class_id,'section_id':this.section_id,'month':this.month}).subscribe(
     data=>{  this.attendanceReport=data;if(this.attendanceReport){this.tableHeader=this.attendanceReport[0].report}},
     error=>this.error=error      
   ) 
 }
