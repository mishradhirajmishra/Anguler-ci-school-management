...
@Component({
  selector: 'app-category-certificate',
  templateUrl: './category-certificate.component.html',
  styleUrls: ['./category-certificate.component.css']
})
export class CategoryCertificateComponent implements OnInit {
 ...
 this.as.chkLogedIn({ 'token': this.token }).subscribe(
  ...
  studentByKey(){
    this.as.studentByKey({'student_key':this.student_key}).subscribe(
      data=>{this.student=data;this.url=this.student.sc_st_certificate;
      },
...
updateProfileImage(){ 
  ...
  formData.append('type','sc_st_certificate');
